...
/**
 * Prepare an image for skin displaying on the model
 * 
 * @param {App} self
 */
function renderImage(self)
{
    var image = new Image();
    ...
    image.onload = function()
    {
        ...
        {
            alert(
                "Given image of size " + w + "px wide and " + h + "px high " +
                "doesn't correspond with a Minecraft skin resoultion (64x32 or " +
                "64x64)!"
            );
        }
    };
...
App.prototype = 
{
    ...
    /**
     * Apply a usernamed skin on the model
     */
    fromUsername: function(username)
    {
        var image = renderImage(this);
        image.src = "https://skins.minecraft.net/MinecraftSkins/" + username + ".png";
    },
    ...
    change: function(name, input)
    {
        ...
        else if (name == "username") this.fromUsername(input.value);
        else if (name == "alex")     this.model.setAlex(input.checked);
