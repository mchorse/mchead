...
App.prototype = 
{
    ...
    change: function(name, input)
    {
        ...
        else if (name == "flat")    input.checked ? this.camera.toOrtho() : this.camera.toPerspec();
        else if (name == "texture") readFile(this, input.files[0]);
        else if (name == "x")       this.mouse.setX(parseInt(input.value));
        else if (name == "y")       this.mouse.setY(parseInt(input.value));
    ...
    {
        dom.$("[name=x]").value = x.toFixed(4);
        dom.$("[name=y]").value = y.toFixed(4);
    },
    
    /**
     * Mouse handler's up hook. This will be triggered when the mouse is up in 
